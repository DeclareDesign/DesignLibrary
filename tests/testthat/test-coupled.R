...
test_that("coupled_help_text matches the Shiny / message note", {
  ...
  expect_equal(
    ResearchDesigns:::coupled_help_text("cluster_random_sampling"),
  ...
  expect_equal(length(ResearchDesigns:::coupled_help_text("multiarm_trial")), 0L)

  args <- data.frame(name = "N", kind = "scalar", stringsAsFactors = FALSE)
  html <- ResearchDesigns:::redesign_kind_help("cluster_random_sampling", args = args)
