#' Create a simlpe design with spillovers
#'
...
#' Intuitively, for ever $1 given to a member of a group, each member receives $1\code{groupsize}
...
simple_spillover_designer <- function(n_groups = 40, group_size = 3, sd =1, code = FALSE)
{
  ...
  design_code <- function() {
    ...
    {{{
      ...
      dgp <- function(i, Z, G) sum(Z[G == G[i]])/n_groups + rnorm(1)*sd
