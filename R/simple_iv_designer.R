...
#' A potential instrument Z is examined, which plausibly causes X. The instrument can be used to assess the effect of X on Y for units whose value of X depends on Z if Z does not negatively affect X for some cases, affects X positively for others,  
#' and affects Y only through X. 
#' 
#' @details 
#' 
#' @param N An integer. Sample size.
#' @param type_prob A vector of four numbers in [0,1]. Probability of each complier type (always-taker, never-taker, complier, defier)
#' @param assign_prob A vector of four numbers in [0,1]. Probability of assignment to encouragement (Z) for each complier type (always-taker, never-taker, complier, defier)
...
#' simple_iv_design_2 <- simple_iv_designer(type_prob = c(.1,.1,.6, .2), b_Y = .5)
...
#' simple_iv_design_4 <- simple_iv_designer(N = 1000, assign_prob = c(.2, .3, .7, .5), b_Y = .5)
#' diagnose_design(simple_iv_design_4)
#' 
#' #' # Generates a simple iv design with violation of first stage
#' simple_iv_design_5 <- simple_iv_designer(type_prob = c(.5,.5, 0, 0), b_Y = .5)
...
simple_iv_designer <- function(N = 100,
                              type_prob   = c(1/3,1/3,1/3,0), # Probabiliy of complier types
                              assign_prob = c(.5,.5,.5,.5),   # Assignment to Z given type
                              a_Y = 1,
                              b_Y = 0,
                              d_Y = 0,
                              sd_Y = 1,
                              a = c(1, 0,0,0)*a_Y, # Constant in Y equation (by type)
                              b = rep(b_Y, 4),     # Slope in Y equation (by type)
                              d = rep(d_Y, 4)      # Exclusion restriction violation by type
){
  u_0 <- Y_Z_1 <- Y_Z_0 <- NULL
  if(min(assign_prob) < 0 ) stop("assign_prob must be non-negative")
  if(max(assign_prob) > 1 ) stop("assign_prob must be < 1")
  {{{
  
    # Model
    population   <- declare_population(
                              N     = N, 
                              type  = sample(1:4, N, replace = TRUE, prob = type_prob),
                              names = c("Always", "Never", "Complier", "Defier")[type],
                              U_Z   = runif(N), 
                              U_Y   = rnorm(N)*sd_Y, 
                              Z     = (U_Z < assign_prob[type]),
                              X     = (type==1) + (type == 3)*Z + (type == 4)*(1-Z)
                              )
    
    potentials <- declare_potential_outcomes(Y ~ a[type] + b[type]*X + d[type]*Z + U_Y, 
    ...
    # Answers
    estimator_1  <- declare_estimator(X ~ Z, estimand = "first_stage", label =  "d-i-m")
    estimator_2  <- declare_estimator(Y ~ X, estimand = c("ate", "late"), 
                             model = lm_robust, label =  "lm_robust")
    estimator_3  <- declare_estimator(Y ~ X | Z, estimand = c("ate", "late"), 
...
attr(simple_iv_designer, "shiny_arguments") <- list(N = c(10, 20, 50), b_Y = c(0,1), d_Y = c(c,1)) 
