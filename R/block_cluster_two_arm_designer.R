...
#' @param N_cluster_in_block Number of clusters per block
...
block_cluster_two_arm_designer <- function(N_blocks = 2,
                                    sd_block = .2,
                                    N_cluster_in_block = 2,
                                    sd_cluster = .2,
                                    N_i_in_cluster = 2,
...
design_code <- function()  {{{
  ...
  # M: Model
   pop <- declare_population(
      blocks   = add_level(N = N_blocks, u_b = rnorm(N)*sd_block),
      clusters = add_level(N = N_cluster_in_block, u_c = rnorm(N)*sd_cluster),
