...
**Code written for DesignLibrary 0.1 mostly keeps running.** Thirteen designers keep their 0.1 names and argument names (`two_arm_designer()`, `two_arm_attrition_designer()`, `pretest_posttest_designer()`, `randomized_response_designer()`, `mediation_analysis_designer()`, `multi_arm_designer()`, `two_by_two_designer()`, `block_cluster_two_arm_designer()`, `two_arm_covariate_designer()`, `regression_discontinuity_designer()`, `spillover_designer()`, `cluster_sampling_designer()`, and `binary_iv_designer()`), each routed to a library design. The last five are new library templates flattened from 0.1 (`two_arm_covariate`, `regression_discontinuity_polynomial`, `spillover`, `cluster_sampling`, and `binary_iv`); at the same arguments and seed, each draws what its 0.1 designer drew. The exception is `cluster_sampling` away from its defaults: `make_design()` builds the fixed population at the defaults before redesigning, so the population has the same distribution but comes from a later point in the random stream. The other two 0.1 designers stop with the related designs to use instead.
