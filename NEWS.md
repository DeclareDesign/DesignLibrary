...
**Code written for DesignLibrary 0.1 mostly keeps running.** Fourteen designers keep their 0.1 names and argument names (`two_arm_designer()`, `two_arm_attrition_designer()`, `pretest_posttest_designer()`, `randomized_response_designer()`, `mediation_analysis_designer()`, `multi_arm_designer()`, `two_by_two_designer()`, `block_cluster_two_arm_designer()`, `two_arm_covariate_designer()`, `regression_discontinuity_designer()`, `spillover_designer()`, `cluster_sampling_designer()`, `binary_iv_designer()`, and `process_tracing_designer()`), each routed to a library design. The last six are new library templates flattened from 0.1 (`two_arm_covariate`, `regression_discontinuity_polynomial`, `spillover`, `cluster_sampling`, `binary_iv`, and `process_tracing_bayes`); at the same arguments and seed, each draws what its 0.1 designer drew. The exception is `cluster_sampling` away from its defaults: `make_design()` builds the fixed population at the defaults before redesigning, so the population has the same distribution but comes from a later point in the random stream. `process_tracing_designer()` fixes its estimator labels at "Straw in the Wind" and "Smoking Gun" and warns when `label_E1` or `label_E2` asks for others. `factorial_designer()` still stops with the related designs to use instead.
