...
```{r, code = get_design_code(crossover_designer(N = 120, a = .5, b = .5, crossover = .2, rho = .6)), eval=TRUE}
```
...
The diagnosis reveals a bias-variance tradeoff. Here, the no crossover effect assumption is violated. This causes some bias in the SUR estimator. However, the correlation in the errors strongly increases the efficiency of the SUR estimator here. Thus, in terms of RMSE the bias is offset by the increased power of the SUR approach. 

## Using the Crossover Designer

In R, you can generate a crossover design using the template function `crossover_designer()` in the `DesignLibrary` package by running the following lines, which install and load the package:

```{r, eval=FALSE}
library(DesignLibrary)
```

We can then create specific designs by defining values for each argument. For example, we create a design called `my_crossover_design` with `N`, `a`, `b`, and `crossover` set to 300, .2, .4, and .15, respectively, by running the lines below.
...
You can see more details on the `crossover_designer()` function and its arguments by running the following line of code:
