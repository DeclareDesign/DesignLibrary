...
vignette: >
  %\VignetteIndexEntry{Two-Arm Experiment with a Covariate}
...
```{r}
designs <- redesign(two_arm_covariate_designer(N = 30, h = .5),
                    rho_WY = c(0, .8), rho_WZ = c(0, .8))
diagnoses <- diagnose_designs(designs, sims = 25)
```

```{r,eval = TRUE, echo = FALSE}
get_diagnosands(diagnoses) %>% 
  arrange(design,match(estimator,c("No controls","With controls","Lin"))) %>% 
  select(c("estimator","rho_WZ","rho_WY","bias","rmse")) %>% 
  kable(digits = 2)
```
