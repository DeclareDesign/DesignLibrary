...
```{r}
# cor_E1E2_H shapes the joint probabilities before any step reads it, so the
# designer is called once per value rather than redesigned.
cors <- c(-.32, 0, .32)
designs <- setNames(lapply(cors, function(r) process_tracing_designer(cor_E1E2_H = r)), cors)

diagnose_designs(designs, sims = 25) %>% 
  get_diagnosands() %>% 
  mutate(cor_E1E2_H = as.numeric(as.character(design))) %>% 
