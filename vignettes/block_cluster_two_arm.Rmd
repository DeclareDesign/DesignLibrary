...
- **M**odel:  We specify a population partitioned into `N\_blocks` (regions, say). Each region has `N\_clusters\_in\_block` clusters (cities, say)  and within each city there are `N\_in\_cluster` individuals.  The variance of an individual's outcomes has three components: the block's variance, the cluster's variance and the subject's variance. 
