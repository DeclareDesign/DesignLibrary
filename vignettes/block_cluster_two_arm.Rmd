...
To better illustrate how different sources of variance differentially affect our power, we compare three designs: one in which all the variance derives from differences between individuals, another in which it derives from differences between clusters, and finally one in which it derives from  

```{r,eval = TRUE}
block_cluster_two_arm_designer(N_blocks = 4, N_clusters_in_block = 4, N_i_in_cluster = 5, sd_cluster = 2)
...
Our diagnosis indicates that the difference-in-means estimator produces unbiased but noisy estimates. In fact, due to the size of its standard error and our sample size, our power is low and thus we would often fail to reject the hyphotesis of null effects.

Now, let us see how our diagnosands change in the presence of large block


```{r,eval = TRUE}
diagnosis <- diagnose_design(block_cluster_two_arm_designer(N_blocks = 4, N_clusters_in_block = 4, N_i_in_cluster = 5, sd_block = 2))
...
Our diagnosis shows that we gain in efficiency when the block variance is large. 
