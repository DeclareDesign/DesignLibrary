...
- **M**odel: 
    ...
    Our model of the world specifies a population of units each of which is a member of a group (`n_groups` of size `group_size` each). The key assumption of this spilover design is that if any member of group, is treated then all units receive some equal benefit (with marginal gains possibly increasing or decreasing in the numbers treated). Specifically, letting $G[i]$ denote the group of which $i$ is a member,  we assume:
...
- **I**nquiry: 
    ...
    The estimand of interest is the average, across all individuals, of the difference between the sitation in which that individual only is treated and one in which no individuals are treated. 
...
In R, you can generate a simple_spillover design using the template function `simple_spillover_designer()` in the `DesignLibrary` package by running the following lines, which install and load the package:
