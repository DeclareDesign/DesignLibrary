...
```{r, echo=FALSE}
diagnosis_table <- get_diagnosands(diagnosis)[, c("estimator_label", "mean_estimand", 
...
And also plot simulated values of our estimand with standard errors not clustered and with clustered standard errors.
...
We can then create specific designs by defining values for each argument. For example, we create a design called `my_cluster_sampling_design` with `N`, `ate`, `rho`, and `attrition_rate` set to 300, .2, .4, and .15, respectively, by running the lines below.

```{r, eval=FALSE}
my_cluster_sampling_design <- cluster_sampling_designer(N = 300,
                                                        ate = .2,
                                                        rho = .4,
                                                        attrition_rate = .15)
```
