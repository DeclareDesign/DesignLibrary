...
Assuming enough variation in the outcome of interest, random cluster assignment yields unbiased but inprecise estimates.^[We assume clusters are the same size. Other designs might include different-sized clusters, but we do not discuss these cases here.] In ordinary least square (OLS) models, we assume errors are independent (error terms between individual observations are uncorrelated with each other) and homoskedastic (that size of errors is homogeneous across individuals). In reality, this is often not the case with cluster sampling. By design, we select groups of individuals who share common attributes which may be correlated with the measure of interest. The higher the degree of within-cluster similarity, the more variance we observe in cluster-level averages and the more imprecise are our estimates under common assumptions. We address this by considering cluster-robust standard errors in our answer strategy below.
