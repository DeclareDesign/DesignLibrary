...
my_design <- 
  ...
  declare_model(N = 100,
                ...
                potential_outcomes(YA ~ YA_0 + ZA * tau_A, conditions = list(ZA = 0:1)),
                potential_outcomes(YB ~ YB_0 + ZB * tau_B, conditions = list(ZB = 0:1))) +
  # Inquiry
  declare_inquiry(ate_YA = mean(YA_ZA_1 - YA_ZA_0)) +
  # Data Strategy
  declare_assignment(ZA = complete_ra(N, m = 50)) +
  declare_assignment(ZB = complete_ra(N, m = 50)) +
  declare_measurement(YA = reveal_outcomes(YA ~ ZA),
                      YB = reveal_outcomes(YB ~ ZB)) +
  # Answer Strategy
  declare_estimator(YA ~ ZA, .method =lm_robust, 
...
```{r, eval = FALSE}
my_design_cor <- redesign(my_design, rho = .8)
