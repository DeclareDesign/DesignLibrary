...
The diagnosis of our design indicates that the difference in means unbiasedly estimates the average treatment effect (ATE) in each arm. These estimates our however not so precise;  The estimated standard deviation is large yielding wide confidence intervals that contain the true value of the ATEs more than 95\% of the times.

## Using the  Multi-Arm Designer

In R, you can generate a simple two arm design using the template function `multi_arm_designer()` in the `DesignLibrary` package by running the following lines, which install and load the package:
