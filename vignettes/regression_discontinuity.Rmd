...
```{r args, code=template_default_args_text(regression_discontinuity_template)}
```

```{r body, code=template_text(regression_discontinuity_template)}
```

## Takeaways 

```{r regression_discontinuity_diagnosis}
diagnosis <- diagnose_design(rd_design, sims = 10000, bootstrap = 1000)
```
